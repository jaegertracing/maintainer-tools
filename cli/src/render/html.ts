...
export function renderHtml(classified: ClassifiedPR[], opts: RenderOptions): string {
  ...
  const body = blocks
    ...
    .join('\n');
  const tabulator = tabulatorAssets();
  const tableView = renderTableView(classified, opts);
...
<style>${tabulator.css}</style>
<style>${CSS}${TABLE_CSS}</style>
<script>${tabulator.js}</script>
</head>
<body>
<header>
  <h1>PR Triage</h1>
...
const CSS = `
  ...
  .flag-COPILOT { background: white; border: 1px solid #d0d7de; }
